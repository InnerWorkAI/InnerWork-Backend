...
class CompanyAnalysisService:
    ...
    async def collect_company_data(company_id: int, db: Session):
        ...
        employees_high_risk = sorted({f.employee_id for f in high_risk_forms})

        trend = CompanyAnalysisService.calculate_trend(company_id, db)
        ...
        for dept_val, scores in department_scores.items():
            ...
            dept_data = {"name": dept_name, "average": round(avg, 2)}
            department_burnout.append(dept_data)
            if avg >= CompanyAnalysisService.HIGH_THRESHOLD:
